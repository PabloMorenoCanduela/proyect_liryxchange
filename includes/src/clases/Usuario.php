...
class Usuario
{
    ...
    public static function crea($correo, $contraseña, $nombre, $genero, $fecha, $pais, $foto_perfil, $tipo)
    {
        $user = new Usuario($correo, self::hashcontraseña($contraseña), $nombre, $genero, $fecha, $pais, $foto_perfil, $tipo); 
        $user->guarda();
        return $user;
    }
    ...
    public static function buscaUsuario($correo)
    {
        ...
        if ($rs) {
            $fila = $rs->fetch_assoc();
            if($fila){
                $user = new Usuario($fila['correo'], $fila['contraseña'], $fila['nombre'], $fila['sexo'], $fila['fecha_nacimiento'], $fila['pais'], $fila['servidor_fotoperfil'], $fila['tipo']);
            }
            $rs->free();
    ...
    public static function devuelveTodosLosUsuarios() {
        ...
        if ($result->num_rows > 0) {
            while ($fila = $result->fetch_assoc()) {
                ...
                $fila['sexo'], $fila['fecha_nacimiento'], $fila['pais'], $fila['servidor_fotoperfil'], $fila['tipo']);
    ...
    public static function buscaPorId($idUsuario)
    {
        ...
        if ($rs) {
            $fila = $rs->fetch_assoc();
            if ($fila) {
                $result = new Usuario($fila['correo'], $fila['contraseña'], $fila['nombre'], $fila['sexo'], $fila['fecha_nacimiento'], $fila['pais'], $fila['servidor_fotoperfil'], $fila['tipo']);
            }
            $rs->free();
    ...
    private static function inserta($usuario)
    {
        $result = false;
        $conn = Aplicacion::getInstance()->getConexionBd();
        $query=sprintf("INSERT INTO usuarios(correo, nombre, contraseña, fecha_registro, sexo, fecha_nacimiento, pais, tipo, servidor_fotoperfil) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')"
    ...
    private $id, $correo, $contraseña, $nombre, $roles, $genero, $fecha, $pais, $foto_perfil, $tipo;
    
    // Constructor privado para evitar instanciación directa
    private function __construct($correo, $contraseña, $nombre, $genero, $fecha, $pais, $foto_perfil, $tipo)
    {
        $this->correo = $correo;
    ...
    public static function datos($correo){
        $conn = Aplicacion::getInstance()->getConexionBd();
        $sql = "SELECT nombre, sexo, fecha_nacimiento, pais FROM usuarios WHERE correo = '$correo'";
